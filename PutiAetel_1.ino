...
RgbColor Rainbow[]={RgbColor(255,0,0),RgbColor(200,80,0),RgbColor(180,180,0),RgbColor(0,255,0),RgbColor(0,0,255),RgbColor(130,0,130)};
struct datosAnimSpectrum{
  RgbColor ArrayOla[50];  //Un array donde podamos meter una ola, que se vuelca
  int WaveSize;
  bool direccion=1; //Esto que se te olvida todo el rato, es si decrece o crece la luminosidad
};
  ...
  if(relojanimacion<millis()){  //Todos esos if me los puedo quitar poniendo las condiciones repetidas antes, y con un switch elejir la animacion
    switch(Animacion){
      ...
      //Serial.println("entramos Spectrum");
       ...
       case 8:
          if(parametro>=1){
            startFadeInFadeOutColor(Rainbow[contadores1[0]]);
            newtamano();
            if(contadores1[0]==5){
              contadores1[0]=0;
            }
  ...
  if (strcmp(tema, "animacion")== 0){
    ...
    switch(Animacion){
      ...
      case 9:
        ...
        Animacion=6;  //El mecanismo es el mismo, pero los colores que coje son los de Rainblow
        break;
    }
  }
...
float startFadeInFadeOutColor(RgbColor color){
  animationState[0].StartingColor=LED.GetPixelColor(0);
  animationState[0].EndingColor=color;
  parametro=0;
}
void newtamano(){
...
void newFasedSpectrum(){
  ...
  animationState[0].frecuencia=float(random(20,100))/700.0;
  ...
  for(int h=0; h<conjuntodeespectros; h++){
    animationState[h+1].frecuencia=float(random(20,100))/700.0;
...
void newFasedSpectrumRainbow(){
  ...
  for(int h=0; h<conjuntodeespectros; h++){
    animationState[h+1].frecuencia=float(random(20,100))/700.0;
    contadores[h]=0.5;  //Vamos a ponerle un valor inicial, para que no se quede atascado por debajo del 0.2, en la siguiente funcion
    animationState[h+1].StartingColor=Rainbow[h%5];
    //Serial.print("FRECUENCIA: ");
    //Serial.println(animationState[h].frecuencia);
    //do{
      //animationState[h+1].StartingColor=RgbColor(random(200),random(200),random(200));
...
void keepFasedSpectrum(){
  ...
  for(int h=0;h<tamano;h++){
    ...
    if(contadores[h]>1||contadores[h]<0.2){
      ...
      contadores[h]=contadores[h]+animationState[h].frecuencia; //Corregimos desviacion superior al valor 255. 
    }
    ...
    //En la anterior funcion... que hay por encima de 255? nada? Bieeennnn. Si multiplicas digamos 1.2*255, sale b100000XX so.. se vueleve un valor inferior, porque la parte superior del byte es redundante
    ...
    if(h==0){ 
      for(int j=PixelCount-posicionamiento;j>0;j++){  //Recuerda 0, no es mas grande que 633
        LED.SetPixelColor(j,RgbColor::LinearBlend(LED.GetPixelColor(PixelCount-posicionamiento),0,float(1.0-float(PixelCount)-float(j)/float(PixelCount))));
        if(j==PixelCount){
          break;
        }
      }
      ...
      for(int j=posicionamiento*(h-1);j<posicionamiento*h;j++){
        LED.SetPixelColor(j,RgbColor::LinearBlend(LED.GetPixelColor(posicionamiento*(h-1)),LED.GetPixelColor(posicionamiento*h),(float(j-posicionamiento*(h-1))/float(posicionamiento))));
        //pff lo anterior no funcionaba (probablemente porque 
        /*Serial.print("posicion colocada: ");
        Serial.print(j);
        Serial.print(" posicion final a colocar: ");
        Serial.println(posicionamiento*h);
        yield();*/
      }
    }
...
/*RgbColor Rainbow(int i){  //No es necesario una funcion para esto, lol. Podriamos poner un RGBObject constante
  switch (i){
    case 0:
